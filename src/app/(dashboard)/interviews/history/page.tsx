...
export default function InterviewsHistoryPage() {
  ...
  const loadedInterviews = data?.pages.flatMap((page) => page.items) || [];
  const interviews = loadedInterviews.filter((item) => (statusFilter === 'all' || item.status === statusFilter) && (typeFilter === 'all' || item.interviewType === typeFilter));
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 space-y-8">
      ...
      <Card variant="elevated" padding="lg" className="space-y-6 bg-white border border-outline-variant/60 shadow-subtle">
        <div className="flex flex-wrap gap-3 border-b border-outline-variant/30 pb-5">
          <label className="text-xs font-semibold">Trạng thái<select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="mt-1 block min-h-10 rounded-lg border border-outline-variant bg-white px-3 text-sm"><option value="all">Tất cả</option><option value="starting">Đang khởi tạo</option><option value="active">Đang diễn ra</option><option value="completing">Đang chấm điểm</option><option value="completed">Hoàn thành</option><option value="failed">Thất bại</option><option value="abandoned">Đã hủy</option></select></label>
          <label className="text-xs font-semibold">Loại phỏng vấn<select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)} className="mt-1 block min-h-10 rounded-lg border border-outline-variant bg-white px-3 text-sm"><option value="all">Tất cả</option><option value="technical">Kỹ thuật</option><option value="behavioral">Hành vi</option><option value="scenario">Tình huống</option><option value="cv_targeted">Theo CV</option><option value="jd_targeted">Theo JD</option></select></label>
          {(statusFilter !== 'all' || typeFilter !== 'all') && <p className="self-end text-xs text-on-surface-variant">Bộ lọc áp dụng cho các phiên đã tải; tải thêm để xem phiên cũ hơn.</p>}
        </div>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-outline-variant/30">
          <div>
            ...
            <p className="text-xs text-on-surface-variant mt-0.5">
              Xem lại trạng thái từng phiên và mở báo cáo khi đã có kết quả đánh giá.
            </p>
          </div>
        ...
        {showInitialLoading ? (
          ...
          <div className="space-y-4 rounded-xl bg-surface-container-low p-8 text-sm text-on-surface-variant"><p>Không có phiên phù hợp trong các trang đã tải. Hãy đổi bộ lọc hoặc tải thêm.</p>{hasNextPage && <Button variant="outline" size="sm" onClick={() => void fetchNextPage()} disabled={isFetchingNextPage}>Tải thêm</Button>}</div>
        ) : (
          <>
