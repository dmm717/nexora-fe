...
import { visibleAboutSections } from '@/services/aboutSections';
...
export default function AboutPage() {
  ...
  const numberFor = (id: typeof sections[number]['id']) => sections.find((section) => section.id === id)!.number;
  return <PublicSiteShell><div className="mx-auto max-w-7xl space-y-16 px-5 py-10 sm:px-8 sm:py-14">
    ...
    <section id="mission" className="grid items-center gap-10 lg:grid-cols-2"><div><p className="text-xs font-bold uppercase tracking-widest text-primary">{numberFor('mission')} / Về Nexora</p><h2 className="mt-3 text-3xl font-extrabold text-[#172554] sm:text-4xl">{content.missionTitle}</h2><p className="mt-5 max-w-xl whitespace-pre-line text-base leading-8 text-[#405176]">{content.missionBody}</p></div><Image src={missionImage} width={1440} height={1080} unoptimized={Boolean(content.missionAssetId)} alt="Người chuẩn bị cho buổi phỏng vấn cùng máy tính và ghi chú" className="aspect-[4/3] w-full rounded-3xl object-cover shadow-card" /></section>
    <section id="values"><p className="text-xs font-bold uppercase tracking-widest text-primary">{numberFor('values')} / Giá trị cốt lõi</p><h2 className="mt-3 text-3xl font-extrabold text-[#172554]">Nexora xây trên điều gì?</h2><div className="mt-6 grid gap-5 md:grid-cols-3">{content.values.map((value) => <article key={value.title} className="rounded-2xl border border-[#dbe3fa] bg-white p-7 shadow-subtle"><h3 className="text-lg font-bold text-[#172554]">{value.title}</h3><p className="mt-3 text-sm leading-7 text-[#52617e]">{value.description}</p></article>)}</div></section>
    {content.milestones.length > 0 && <section id="milestones"><p className="text-xs font-bold uppercase tracking-widest text-primary">{numberFor('milestones')} / Hành trình phát triển</p><h2 className="mt-3 text-3xl font-extrabold text-[#172554]">Những cột mốc của Nexora</h2><div className="mt-6 grid gap-4 md:grid-cols-2">{content.milestones.map((item) => <article key={`${item.label}-${item.title}`} className="rounded-2xl border border-[#dbe3fa] bg-white p-6"><span className="text-sm font-bold text-primary">{item.label}</span><h3 className="mt-2 font-bold text-[#172554]">{item.title}</h3><p className="mt-2 text-sm leading-7 text-[#52617e]">{item.description}</p></article>)}</div></section>}
    {content.teamSectionEnabled && content.teamMembers.length > 0 && <section id="team"><p className="text-xs font-bold uppercase tracking-widest text-primary">{numberFor('team')} / Đội ngũ</p><h2 className="mt-3 text-3xl font-extrabold text-[#172554]">{content.teamHeading || 'Đội ngũ Nexora'}</h2><div className="mt-6 grid gap-5 md:grid-cols-3">{content.teamMembers.map((member) => <article key={`${member.name}-${member.role}`} className="rounded-2xl border border-[#dbe3fa] bg-white p-6">{member.assetId && <Image src={siteAssetUrl(member.assetId)} width={400} height={400} unoptimized alt={member.name} className="mb-4 aspect-square w-full rounded-xl object-cover" />}<h3 className="font-bold">{member.name}</h3><p className="text-sm text-primary">{member.role}</p>{member.bio && <p className="mt-2 text-sm text-[#52617e]">{member.bio}</p>}</article>)}</div></section>}
    <section id="ecosystem" className="pb-6"><p className="text-xs font-bold uppercase tracking-widest text-primary">{numberFor('ecosystem')} / Nexora hỗ trợ bạn như thế nào</p><h2 className="mt-3 text-3xl font-extrabold text-[#172554]">Một hành trình luyện tập liền mạch</h2><div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{capabilities.map(({ title, href, icon: Icon }) => <Link key={title} href={href} className="flex items-center gap-3 rounded-2xl border border-[#dbe3fa] bg-white p-5 text-sm font-bold text-[#172554] hover:border-primary hover:text-primary"><Icon size={20} aria-hidden="true" />{title}<ArrowRight size={16} className="ml-auto" aria-hidden="true" /></Link>)}</div></section>
  </div></PublicSiteShell>;
}
